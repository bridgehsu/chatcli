...
/// Facts returned from local filesystem search. The caller decides whether a
/// result is safe to use for terminal creation.
#[derive(Debug)]
...
fn search_directories(
    ...
    let matches = search(roots, query, max_depth, true);
    Ok(to_result(matches))
}

fn search_files(roots: &[PathBuf], query: &str, max_depth: usize) -> Result<SearchResult, String> {
    let matches = search(roots, query, max_depth, false);
    Ok(to_result(matches))
}
